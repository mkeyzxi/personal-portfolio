...
function AboutContent() {
  const {data: aboutData} = useSWR<AboutData>(
    '/api/about',
    fetcher,
    {revalidateOnFocus: false, dedupingInterval: 60000 * 60, suspense: true}, // suspense: true
  )
      ...
      <motion.div variants={rightSlideVariants} className="w-full lg:w-2/3 flex flex-col gap-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
          <motion.div
            ...
            className="flex flex-col gap-2 rounded-[24px] border border-[var(--color-border)]/50 bg-[var(--color-bg-surface)] p-6 transition-colors hover:border-[var(--color-text-muted)]"
          >
