      ...
      if (typeof navigator !== 'undefined' && !navigator.onLine) {
        ...
        if (view === 'create') {
          setStories((prev) => [
            ...
            } as unknown as import('@/types').StoryDocument,
            ...prev,
          ])
        }
        return
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-[var(--color-text-primary)]">
            Cerita & Blog (Story CMS)
          </h1>
          <p className="text-sm text-[var(--color-text-secondary)] mt-1">
            Tulis dan kelola artikel atau cerita Anda.
          </p>
        </div>
        {view === 'list' && (
          <button
          ...
          >
            <Icon icon="lucide:plus" className="w-5 h-5" /> Buat Cerita
          </button>
        )}
      </div>
              ...
              <tbody>
                {stories.map((story) => (
                  <tr key={story.id} className="border-t border-[var(--color-border)] align-top">
                    ...
                    <td className="p-4">
                      <span
                        className={`px-2 py-1 rounded text-xs font-semibold ${story.status === 'published' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}
                      >
                        {story.status}
                      </span>
                    </td>
                    ...
                    <td className="p-4 text-right space-x-2">
                      ...
                      <button
                        ...
                        title="Delete"
                      >
                        <Icon icon="lucide:trash" className="w-5 h-5 inline-block" />
                      </button>
                    </td>
                  </tr>
                ))}
        ...
        <div className="bg-[var(--color-bg-surface)] p-6 md:p-8 border border-[var(--color-border)] rounded-xl space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex flex-col gap-2">
              ...
              <input
                type="text"
                placeholder="Misal: Perjalanan Karir Saya"
                value={title}
            ...
            <div className="flex flex-col gap-2">
              ...
              <input
                type="text"
                placeholder="Otomatis berdasarkan judul..."
                value={slug}
          ...
          <div className="flex flex-col gap-2">
            ...
            <textarea
              placeholder="Ceritakan sedikit tentang artikel ini..."
              value={summary}
          ...
          <div className="mt-4 flex flex-col gap-2">
            <label className="text-sm font-medium text-[var(--color-text-secondary)]">
              Konten Artikel (BlockNote Editor) *
            </label>
          ...
          <div className="flex justify-end gap-4 pt-6 border-t border-[var(--color-border)] mt-8">
            ...
            >
              <Icon icon="lucide:save" className="w-5 h-5" /> Simpan Cerita
            </button>
          </div>
        </div>
      )}
