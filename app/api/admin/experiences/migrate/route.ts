...
export async function GET() {
  try {
    ...
    // Data from NewFeatExperience.md and data/experiences.ts
    const seedData: Omit<Experience, 'id'>[] = [
      {
        type: 'work',
    ...
    // First, clear existing to avoid duplicates if run multiple times
    const existing = await adminDb.collection('experiences').get();
    ...
    return NextResponse.json({ success: true, message: 'Migration completed successfully!' }, { status: 200 });
  } catch (error: any) {
